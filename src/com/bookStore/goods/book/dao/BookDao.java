...
//		sql = "select * from book" + whereSql + " order by orderBy limit ?,?";
		...
		sql = "select top "+ps+" * from book where bid not in ( "
				+"select top "+((pc-1) * ps)+" bid from book" + whereSql + " order by orderBy ) "+whereSql1+" order by orderBy";
...
//		params.add(ps);//一共查询几行，就是每页记录数
	...
	public List<Book> bookRankingList() throws SQLException {
		String sql = "SELECT TOP 6 * from book where bid in (SELECT top 100 percent s.bid FROM [order] p,orderitem s where "
				+ "p.oid = s.oid AND p.status = 4 GROUP BY bid order by SUM(s.quantity) desc )";
