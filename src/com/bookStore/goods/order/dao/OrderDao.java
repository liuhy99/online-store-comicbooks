...
public class OrderDao {
	...
	public void add(Order order) throws SQLException {
		...
		String sql = "insert into [order] ([oid],[ordertime],[total],[status],[address],[uid]) values (?,?,?,?,?,?)";
		Object[] params = {order.getOid(), order.getOrdertime(),
				order.getTotal(),order.getStatus(),order.getAddress(),
				order.getOwner().getUid()};
		qr.update(sql, params);

		sql = "insert into orderitem ([orderItemId],[quantity] ,[subtotal],[bid],[bname],[currPrice],[image_b],[oid]) values (?,?,?,?,?,?,?,?)";
	...
	private PageBean<Order> findByCriteria(List<Expression> exprList, int pc) throws SQLException {
		...
		StringBuilder whereSql1 = new StringBuilder(" and 1=1"); 
		for(Expression expr : exprList) {
			whereSql1.append(" and ").append(expr.getName())
				.append(" ").append(expr.getOperator()).append(" ");
			if(!expr.getOperator().equals("is null")) {
				whereSql1.append("?");
				params.add(expr.getValue());
		...
		sql = "select top " + ps +" * from [order] where oid not in ( "
				+"select top "+((pc-1) * ps)+" oid from [order]" + whereSql + " order by ordertime )"+whereSql1+" order by ordertime";
